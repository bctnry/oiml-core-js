...
function _takeStringOrNumber(x: string, line: number = -1): [string|number|null, string] {
...
function _takeSimpleValue(x: string, line: number = -1): [SimpleValue|null, string] {
    let first: [string|number|null, string] = _takeStringOrNumber(x, line);
    if (!first[0]) { return [null, x]; }
    if (!first[1].startsWith('=')) { return first; }
    let second: [string|number|null, string] = _takeStringOrNumber(first[1].substring(1), line);
...
function _parseHead(requiredIndent: number, currentLine: number, x: string): Block {
    ...
    while (subj) {
        ...
        assert(subj[0] && subj[0] === ':', 'non separator should already be parsed as simple value');
        if (subj[0] && subj[0] === ':') {
